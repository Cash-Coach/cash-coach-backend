...
public class ExcelService {
    ...
    public void writeIncomesToExcel(OutputStream os, List<IncomeDTO> incomeList) throws IOException {
        try (Workbook wb = new XSSFWorkbook()) {
            Sheet sheet = wb.createSheet("Income_Records");
            ...
            IntStream.range(0, incomeList.size())
                    .forEach(i -> {
                        IncomeDTO income = incomeList.get(i);
    ...
    public void writeExpensesToExcel(OutputStream os, List<ExpenseDTO> expenseList) throws IOException {
        try (Workbook wb = new XSSFWorkbook()) {
            Sheet sheet = wb.createSheet("Expense_Records");
            Row header = sheet.createRow(0);
            header.createCell(0).setCellValue("S.No");
            header.createCell(1).setCellValue("Name");
            header.createCell(2).setCellValue("Category");
            header.createCell(3).setCellValue("Amount");
            header.createCell(4).setCellValue("Date");
            IntStream.range(0, expenseList.size())
                    .forEach(i -> {
                        ExpenseDTO expense = expenseList.get(i);
                        Row row = sheet.createRow(i + 1);
                        row.createCell(0).setCellValue(i + 1);
